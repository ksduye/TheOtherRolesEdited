    ...
    public static class Helpers
    {
        ...
        public static Texture2D loadTextureFromDisk(string path) {
            try {          
                ...
                TheOtherRolesEditedPlugin.Logger.LogError("Error loading texture from disk: " + path);
            }
            return null;
        }
        ...
        public static AudioClip loadAudioClipFromResources(string path, string clipName = "UNNAMED_TOR_AUDIO_CLIP") {
            ...
            /* Usage example:
            AudioClip exampleClip = Helpers.loadAudioClipFromResources("TheOtherRolesEdited.Resources.exampleClip.raw");
        ...
        internal static string getRoleString(RoleInfo roleInfo)
        {
            if (roleInfo.name == "豺狼") 
            {
                var getSidekickText = Jackal.canCreateSidekick ? " 并招募一名跟班" : "";
                return cs(roleInfo.color, $"{roleInfo.name}: 击杀所有人{getSidekickText}");  
            }
        ...
        public static void shareGameVersion() {
            ...
            writer.Write((byte)TheOtherRolesEditedPlugin.Version.Major);
            writer.Write((byte)TheOtherRolesEditedPlugin.Version.Minor);
            writer.Write((byte)TheOtherRolesEditedPlugin.Version.Build);
            writer.Write(AmongUsClient.Instance.AmHost ? Patches.GameStartManagerPatch.timer : -1f);
            writer.WritePacked(AmongUsClient.Instance.ClientId);
            writer.Write((byte)(TheOtherRolesEditedPlugin.Version.Revision < 0 ? 0xFF : TheOtherRolesEditedPlugin.Version.Revision));
            writer.Write(Assembly.GetExecutingAssembly().ManifestModule.ModuleVersionId.ToByteArray());
            AmongUsClient.Instance.FinishRpcImmediately(writer);
            RPCProcedure.versionHandshake(TheOtherRolesEditedPlugin.Version.Major, TheOtherRolesEditedPlugin.Version.Minor, TheOtherRolesEditedPlugin.Version.Build, TheOtherRolesEditedPlugin.Version.Revision, Assembly.GetExecutingAssembly().ManifestModule.ModuleVersionId, AmongUsClient.Instance.ClientId);
        }
        ...
        public static void toggleZoom(bool reset=false) {
            ...
            if (HudManagerStartPatch.zoomOutButton != null) {
                HudManagerStartPatch.zoomOutButton.Sprite = zoomOutStatus ? Helpers.loadSpriteFromResources("TheOtherRolesEdited.Resources.PlusButton.png", 75f) : Helpers.loadSpriteFromResources("TheOtherRolesEdited.Resources.MinusButton.png", 150f);
        ...
        public static async Task checkBeta() {
            if (TheOtherRolesEditedPlugin.betaDays > 0) {
                ...
                TheOtherRolesEditedPlugin.Logger.LogMessage($"Compiled at {compileTime.ToString(CultureInfo.InvariantCulture)}");
                DateTime? now;
                ...
                try {
                    ...
                    if (response.IsSuccessStatusCode)
                        ...
                        TheOtherRolesEditedPlugin.Logger.LogMessage($"Could not get time from server: {response.StatusCode}");
                        now = DateTime.UtcNow; //In case something goes wrong. 
                    }
                } catch (System.Net.Http.HttpRequestException) {
                    now = DateTime.UtcNow;
                }
                if ((now - compileTime)?.TotalDays > TheOtherRolesEditedPlugin.betaDays) {
                    TheOtherRolesEditedPlugin.Logger.LogMessage($"Beta expired!");
        ...
        public static void enableCursor(string mode)
        {
            if (mode == "start")
            {
                Sprite sprite = Helpers.loadSpriteFromResources("TheOtherRolesEdited.Resources.CursorButton.png", 115f);
                Cursor.SetCursor(sprite.texture, Vector2.zero, CursorMode.Auto);
                return;
            }
            if (TheOtherRolesEditedPlugin.TheOtherRolesEditedCursor.Value)
            {
